...
async function main() {
  ...
  while (index <= length) {
    ...
    characterMetadata.name = (await contract.getName(index)) || `${TRAIT_VALUE_MAP[characterTraits[0]]} #${index}`;
    ...
    if (fs.existsSync(filename)) {
      ...
      characterTraits.forEach((trait, i) => {
        if (trait !== 0) {
          console.log(CHARACTER_TRAIT_TYPES[i], trait);
          characterMetadata.attributes.push({ trait_type: CHARACTER_TRAIT_TYPES[i], value: TRAIT_VALUE_MAP[trait] });
        }
      });

...
function Swap({ selectedProvider, tokenListURI }) {
  ...
  useEffect(() => {
    const getTokenList = async () => {
      ...
      try {
        const thisTokenList = await fetch(_tokenListUri);
        const tokenListJson = await thisTokenList.json();
      ...
      >
        <Descriptions title="Details" column={1} style={{ textAlign: "left" }}>
          ...
          <Descriptions.Item label="bestPrice">
            {trades && trades.length > 0 ? trades[0].executionPrice.toSignificant(6) : null}
          </Descriptions.Item>
          <Descriptions.Item label="nextMidPrice">
            {trades && trades.length > 0 ? trades[0].nextMidPrice.toSignificant(6) : null}
          </Descriptions.Item>
          <Descriptions.Item label="priceImpact">
            {trades && trades.length > 0 ? trades[0].priceImpact.toSignificant(6) : null}
          </Descriptions.Item>

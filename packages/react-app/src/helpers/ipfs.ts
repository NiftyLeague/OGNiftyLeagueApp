...
export function makeImageGatewayURL(ipfsURI: string): string {
  return `${process.env.REACT_APP_IPFS_GATEWAY as string}/ipfs/${ipfsURI.slice('ipfs://'.length)}`;
}

export async function ResolveImageURL(tokenId: number | string): Promise<string | null> {
